...
#define mutex_assert_locked(mtx) ({                                   \
    ...
    spin_lock(&(mtx)->mtx_gaurd);                                     \
    int locked = ((mtx)->mtx_lock && ((mtx)->mtx_thread == current)); \
    assert(locked, "[tid:%d:pid:%d on cpu:%d] must hold mutex.\n",    \
           gettid(), getpid(), getcpuid());                           \
    spin_unlock(&(mtx)->mtx_gaurd);                                   \
})
...
#define mutex_lock(mtx) ({                                                                     \
    ...
    spin_lock(&(mtx)->mtx_gaurd);                                                              \
    int locked = ((mtx)->mtx_lock && ((mtx)->mtx_thread == current));                          \
    assert(!locked, "[tid:%d:pid:%d on cpu:%d] mutex already held @ %s:%s:%d.\n",              \
           gettid(), getpid(), getcpuid(), (mtx)->mtx_func, (mtx)->mtx_file, (mtx)->mtx_line); \
    ++(mtx)->mtx_lock;                                                                         \
    if ((mtx)->mtx_lock > 1) {                                                                 \
        current_lock();                                                                        \
    ...
    assert_ne((mtx)->mtx_lock, 0, "mutex->mtx_lock == 0??");                                   \
